...
(function(){
  ...
  document.addEventListener('click',function(e){
    ...
    const href=a.getAttribute('href')||''; const label=(a.textContent||'').trim().slice(0,100);
    if(href.includes('/custom-crystal-bracelet-specifications'))track('specifications_click',{event_category:'evidence',event_label:label,link_url:href});
    if(href.includes('/content-standards'))track('content_standards_click',{event_category:'trust',event_label:label,link_url:href});
    if(href.includes('gia.edu')||href.includes('ftc.gov'))track('authority_source_click',{event_category:'trust',event_label:label,link_url:href});
  },true);
})();
...
(function(){
  function track(name,params){try{if(typeof gtag==='function')gtag('event',name,params||{})}catch(e){}}
  document.addEventListener('click',function(e){
    const a=e.target.closest&&e.target.closest('.ai-answer-card a'); if(!a)return;
    track('ai_direct_answer_click',{event_category:'ai_search_support',event_label:(a.textContent||'').trim(),link_url:a.getAttribute('href')||''});
  },true);
})();
